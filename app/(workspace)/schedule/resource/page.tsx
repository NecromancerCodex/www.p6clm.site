...
const opChip: React.CSSProperties = { background: "#eef2ff", color: "#4338ca", borderRadius: 4, padding: "1px 6px", fontSize: 11, whiteSpace: "nowrap" };
const eqChip: React.CSSProperties = { display: "inline-block", background: "#fef3c7", color: "#92400e", borderRadius: 4, padding: "1px 5px", fontSize: 10.5, marginRight: 4, marginBottom: 2 };
...
export default function ResourcePlanPage() {
  ...
  return (
    <div className="ws-inner-pad" style={{ maxWidth: "none" }}>
      <div className="ws-section-title"><Boxes size={18} strokeWidth={1.8} /> 자원 계획</div>
      ...
      <div style={{ ...CARD, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <span style={{ fontSize: 13, fontWeight: 700, color: "#334155" }}>공정계획</span>
      ...
      {equipTotal.length > 0 && (
        <div style={{ ...CARD, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: 13, fontWeight: 700, color: "#92400e" }}>🚜 장비 동원(최대 동시)</span>
          {equipTotal.map(([name, n]) => (
            <span key={name} style={{ background: "#fef3c7", color: "#92400e", borderRadius: 6, padding: "2px 8px", fontSize: 12 }}>
              {name} <b>{n}대</b>
            </span>
          ))}
        </div>
      ...
      {rows.length > 0 ? (
        Object.entries(byDisc).map(([disc, list]) => (
          <div key={disc} style={CARD}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: "#1e293b", margin: "0 0 10px" }}>
              {disc} <span style={{ color: "#94a3b8", fontWeight: 400 }}>· {list.length}개 활동</span>
            </h3>
                ...
                <tbody>
                  {list.map((r) => (
                    <tr key={r.code} style={{ borderTop: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ ...td, fontWeight: 600, color: "#334155" }}>{r.name}</td>
                      <td style={{ ...td, color: "#64748b" }}>{[r.zone, r.storey].filter(Boolean).join(" / ") || "—"}</td>
        ...
        !loading && planId && <div style={{ ...CARD, color: "#94a3b8", textAlign: "center" }}>이 플랜에 활동이 없습니다.</div>
      )}
