...
const DISC_EQUIP: Record<string, string[]> = {
  ...
  // 구조 = 크레인 하나가 아니라 양중 체계 — 타워크레인(주양중)+이동식(철골 보조)+호이스트(자재 수직운반)+고소작업대.
  구조: ["콘크리트펌프카", "크레인", "이동식크레인", "호이스트(리프트)", "고소작업대", "작업조"],
  종합: ["콘크리트펌프카", "크레인", "이동식크레인", "호이스트(리프트)", "고소작업대", "작업조"],
  건축: ["이동식크레인", "호이스트(리프트)", "고소작업차(스카이)", "작업조"],
  MEP: ["고소작업차(스카이)", "작업조"],
  조경: ["굴삭기(백호)", "덤프트럭", "작업조"],
  가설: ["크레인", "호이스트(리프트)", "작업조"],
};
  ...
  const handleBoqUpload = async (cardKey: string, file: File | undefined) => {
    ...
    try {
      ...
      if (cardKey === "구조" || cardKey === "종합") {
        ...
        // 동반 양중 장비(결정론 파생, 수정 가능) — 이동식=타워의 절반(철골 보조), 호이스트=물량 스케일, 고소대=구역당 1
        auto["이동식크레인"] = clamp(Math.round(auto["크레인"] / 2), 1, 6);
        auto["호이스트(리프트)"] = clamp(Math.round(conc / 60000), 1, 4);
        auto["고소작업대"] = clamp(auto["작업조"], 2, 8);
        // 존 상한은 여기서 불가 — IFC 존은 '생성 시 분석'에서야 확보(내역서 시점엔 항상 빈 상태).
        // 존 확보 시점의 소급 교정 effect(아래 useEffect [zones])가 유일한 상한 경로.
      } else if (cardKey !== "토목") {
        // 건축·MEP·조경: 자기 물량(마감㎡·설비m 합)으로 작업조 스케일. 토목은 굴삭기=장비 세트.
        auto["작업조"] = clamp(Math.round(itemSum / 25000), 3, 12);
        if (cardKey === "건축") { auto["호이스트(리프트)"] = clamp(Math.round(auto["작업조"] / 4), 1, 3); auto["고소작업차(스카이)"] = clamp(Math.round(auto["작업조"] / 2), 1, 6); }
        if (cardKey === "MEP") auto["고소작업차(스카이)"] = clamp(Math.round(auto["작업조"] / 2), 1, 6);
        if (cardKey === "조경") auto["덤프트럭"] = clamp((auto["굴삭기(백호)"] || 1) * 2, 2, 8);
      }
