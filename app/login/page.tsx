"use client";

import { Suspense, useEffect, useState } from "react";
import { useRouter, useSearchParams } from "next/navigation";

import { fetchMe, googleLoginUrl } from "../../lib/auth";

const ERROR_MESSAGES: Record<string, string> = {
  oauth: "Google 인증이 취소되었거나 실패했습니다.",
  token: "Google 토큰 교환에 실패했습니다.",
  userinfo: "Google 프로필을 가져오지 못했습니다.",
  forbidden: "접근이 허용되지 않은 계정입니다. 관리자에게 문의하세요.",
};

// useSearchParams() 는 Suspense 경계 필수 (Next App Router prerender 요건).
function LoginInner() {
  const router = useRouter();
  const params = useSearchParams();
  const errorCode = params.get("error");
  const [checking, setChecking] = useState(true);

  // 이미 로그인 상태면 홈으로.
  useEffect(() => {
    let alive = true;
    fetchMe().then((user) => {
      if (!alive) return;
      if (user) router.replace("/");
      else setChecking(false);
    });
    return () => {
      alive = false;
    };
  }, [router]);

  if (checking) {
    return (
      <div className="login-screen">
        <div className="login-card">
          <div className="login-spinner" aria-label="확인 중" />
        </div>
      </div>
    );
  }

  return (
    <div className="login-screen">
      <div className="login-card">
        <div className="login-logo">
          <span className="login-logo-mark">⛏</span>
          <strong>p6 CLM</strong>
          <span className="login-sub">건설 현장 AI</span>
        </div>

        {errorCode && (
          <div className="login-error" role="alert">
            {ERROR_MESSAGES[errorCode] ?? "로그인 중 오류가 발생했습니다."}
          </div>
        )}

        <p className="login-desc">계속하려면 Google 계정으로 로그인하세요.</p>

        <a className="login-google-btn" href={googleLoginUrl()}>
          <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden>
            <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z" />
            <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z" />
            <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z" />
            <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z" />
          </svg>
          Google로 로그인
        </a>

        <p className="login-footer">
          로그인하면 HttpOnly 쿠키 세션으로 안전하게 인증됩니다.
        </p>
      </div>
    </div>
  );
}

export default function LoginPage() {
  return (
    <Suspense
      fallback={
        <div className="login-screen">
          <div className="login-card"><div className="login-spinner" aria-label="로딩 중" /></div>
        </div>
      }
    >
      <LoginInner />
    </Suspense>
  );
}
