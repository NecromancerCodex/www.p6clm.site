...
export interface ProcElement {
  globalId: string;
  name?: string; // 부재 이름 — 흙막이 벽체(CIP·SCW) 정밀 매칭 등 이름 신호용
  trade?: string;
  zone?: string;
  storey4d?: string;
  wt?: string;
...
const _FINISH_NM_EL = /단열|그라스울|글라스울|석고보드|석고|벽지|도배|징두리|걸레받이|몰딩|루버|미장|뿜칠|방수시트|점토벽돌|치장벽돌|데코타일|아트월|코너비드|쇠흙손|배수판|방통|레벨링|바닥마감|A_FIN|A_INS|마감재|건식벽|방습벽|경량벽|칸막이/i;
...
// "토목-" 접두 = 모델러의 명시 공종 태그(실측: "바닥:토목-Asphalt Concrete"가 골조 층창에 오매칭).
const _TEMP_NM4 = /비계|scaffold|동바리|shoring|잭서포트|가설|거푸집|갱폼|유로폼|알폼/i;
const _CIVIL_NM4 = /토목|아스팔트|asphalt|흙막이|토류|버팀|띠장|복공판|cip|scw|어스앵커|엄지말뚝|h-?pile|지하연속벽|diaphragm|slurry|옹벽|측구|집수정|암거|보강토/i;
export function classifyDisc(
  ...
  // 가설 — 이름 신호(비계·동바리 등). 분기 부재 시 fileDisc(구조)로 흘러 골조 층창에 색칠되는 실측 버그.
  if (el.trade === "TW" || _TEMP_NM4.test(nm)) return "가설";
  // 건축 — 타입 + 마감 자재명(_FINISH_NM_EL). 구조 슬롯에 섞인 건식벽체 등도 건축으로.
  if (el.trade === "AR" || _FINISH_TYPES.has(el.ifcType) || _FINISH_NM_EL.test(nm)) return "건축";
...
export function matchAll(
  ...
  for (const el of elements) {
    ...
    if (el.disc === "가설" || el.trade === "TW") {   // 가설 — 골조 매칭 금지, 미매칭(4D 오버레이 담당)
      ranges.set(el.globalId, { range: null, via: "temp:overlay" });
      byVia["가설"] = (byVia["가설"] ?? 0) + 1;
      continue;
    }
    if (el.disc === "토목" || el.trade === "CV") {
